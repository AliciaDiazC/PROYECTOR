...
library(reshape2)
Votos4<- Votos %>%
  select(Provincia, c(11:26)) 
Votos4 <- melt(Votos4, id.vars = c("Provincia")) ## cambiar el nombre de variable por partidos y value por votos
Prov_part <- dcast(Votos4, Provincia ~ variable, fun.aggregate = sum, value.var = "value")
Prov_part
```
Veamos cual gana. 
```{r part}
part <- t(Prov_part)
colnames(part) <- c("Álava", "Guipúzcoa", "Vizcaya")
part<- part[c(2:nrow(part)),]
```
