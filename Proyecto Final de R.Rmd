...
### Rpubs
Esta pagina web, <http://rpubs.com/ydmarinb/429756> me gusta mucho, me parece que viene muy bien explicado y estructurado. Al empezar da una breve explicación sobre que son las expresiones regulares. Además, está estructurado de una manera para que el lector lo comprenda al momento, y si quiere algo en especial, solo tiene que pinchar en el índice. En cada parte viene con un ejemplo para enseñar como se usaría cada expresión. Y además está creado con Rmarkdown.

### Diego Calvo
La segunda es <http://www.diegocalvo.es/expresiones-regulares-en-r/>, comparado con la anterior me parece que la estructura no es tan buena. En mi opinión, le falta una pequeña explicacción para los que no saben que nada sobre expresiones regulares, y se centra menos en la explicación de cada función. 
...
### Rpubs
Desde mi punto de vista, <https://rpubs.com/JohanMarin/Rmarkdown> es más visual que el anterior. En contenidos explican más o menos lo mismo, sin embargo, este explica como incluir imágenes, código in-line, strings externos. Aunque la estructura no es lo mejor, ya que explica como crear tablas o listas, antes de explicar lo más principal. Un punto a su favor es que esta hecho en Rmarkdown.
...
v  <-  ggplot(Votos, aes(x = Provincia, y = Porcentaje_participacion)) +geom_violin(trim=FALSE, fill="Darkslategray3")+
...
En esta sección se estudiará cuales son los partidos políticos más votados en cada provincia. Para ello se creará el siguiente diagrama de barras, en el cual en cada provincia se contabiliza el porcentaje de votos respecto a la población de cada una. Se puede ver que el partido ganador en las tres provincias es el PNV(Partido Nacionalista Vasco), seguido por BILDU en Vizcaya Y Guipúcua. En Álava no hay una gran diferencia entre el PP y BILDU, ganando el PP. Además hay partidos que no consiguen ni un voto, como Vox en Vizcaya.
...
Los tres partidos ganadores de cada provincia se pueden distinguir en la siguiente tabla.  
...
knitr:: kable(rank_part) %>%
  kable_styling(full_width = F, font_size = 10)
```

A continuación, hemos hallado los partidos políticos que han ganado en cada distrito, alguno de ellos se ven reflejados en la siguiente tabla .
...
knitr::kable(head(rank_voto)) %>%
  kable_styling(full_width = F, font_size = 10) #position = "float_right")
```
Solo me falta el mapa.
```{r coord, echo=FALSE, message=FALSE, warning=FALSE}
coord <- c(coord$geometry.type, coord$geometry.coordinates)
```

```{r cod, echo=FALSE, message=FALSE, warning=FALSE}
#library(leaflet)
#library(sp)
#library(maptools)
#geo <- sptable(Votos$codine)
#sp.polygons(Votos$codine)
#[,c(1,5,6)] #Extraemos las coordenadas de los poligonos
