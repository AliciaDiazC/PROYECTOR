...
No se encuentra ninguna relación entre el número de habitantes y de votos ya que en gráfico de puntos se observa que los pueblos pequeños tienen mayor porcentaje de voto. Y sin embargo en Álava que es donde más pueblos pequeños y medianos hay se vota menos en las otras dos provincias.

((((((()))))))
OBS: los pueblos pequeños que más votan son los de guipuzcoa y los que menos votan son los de alava
Vizcaya vota más y tiene menos pueblos.

```{r pueblos, echo=FALSE, message=FALSE, warning=FALSE}
Votos3 <- Votos %>%
  select(Provincia, Rank_censo, Porcentaje_participacion) %>%
  filter(Provincia == "Guipúzcoa") %>%
  arrange(Porcentaje_participacion)
rbind(head(Votos3,10), tail(Votos3,10))
```
```{r pueblos2, echo=FALSE, message=FALSE, warning=FALSE}
Votos3 <- Votos %>%
  select(Provincia, Rank_censo, Porcentaje_participacion) %>%
  filter(Provincia == "Álava") %>%
  arrange(Porcentaje_participacion)
rbind(head(Votos3,10), tail(Votos3,10))
```
```{r pueblos3, echo=FALSE, message=FALSE, warning=FALSE}
Votos3 <- Votos %>%
  select(Provincia, Rank_censo, Porcentaje_participacion) %>%
...
rbind(head(Votos3,10), tail(Votos3,10))
```
Cuanto menos habitantes mas votan, en vizcaya los que menos vtan son los que mas habitantes tienen
((((((()))))))

Grafico, nulos, abstenciones, blancos, provincias, facetas.
